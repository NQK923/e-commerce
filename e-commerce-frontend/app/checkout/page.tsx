      ...
      <div className="rounded-md border border-zinc-200 bg-white p-6 shadow-none">
        ...
        <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
          <div className="grid gap-4 sm:grid-cols-2">
             <div 
               className={`cursor-pointer rounded-md border p-4 transition-all ${paymentMethod === 'COD' ? 'border-black bg-zinc-50 ring-1 ring-black' : 'border-zinc-200 hover:border-black'}`}
               onClick={() => setPaymentMethod('COD')}
             >
               <div className="flex items-center gap-3">
                 <div className={`flex h-5 w-5 items-center justify-center rounded-full border ${paymentMethod === 'COD' ? 'border-black' : 'border-zinc-300'}`}>
                   {paymentMethod === 'COD' && <div className="h-2.5 w-2.5 rounded-full bg-black" />}
                 </div>
                 <span className="font-medium text-zinc-900">Cash on Delivery</span>
               </div>
             </div>
             <div 
               className={`cursor-pointer rounded-md border p-4 transition-all ${paymentMethod === 'VNPAY' ? 'border-black bg-zinc-50 ring-1 ring-black' : 'border-zinc-200 hover:border-black'}`}
               onClick={() => setPaymentMethod('VNPAY')}
             >
               <div className="flex items-center gap-3">
                 <div className={`flex h-5 w-5 items-center justify-center rounded-full border ${paymentMethod === 'VNPAY' ? 'border-black' : 'border-zinc-300'}`}>
                   {paymentMethod === 'VNPAY' && <div className="h-2.5 w-2.5 rounded-full bg-black" />}
                 </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="rounded-md bg-black px-4 py-3 text-sm font-semibold text-white transition hover:bg-zinc-800 disabled:opacity-60 shadow-none w-full"
            >
