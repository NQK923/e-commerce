...
- [~] Add automated tests for critical backend and frontend flows. Initial backend regression tests now cover CORS dev origins, dev-safe OAuth callback gating, VNPay payment ownership and callback verification, notification mark-read ownership, chat send/delivery authorization, seller product mutation ownership/preservation, seller application review, product report admin actions, admin flash sale mutation validation/cache warm-up, Elasticsearch adapter mapping/query construction, and daily sales report aggregation; broader service/UI regression coverage remains open.
...
- [~] Notifications/chat: runtime smoke covered notification record/list/unread/mark-read ownership and chat conversation list. Chat send/delivery now has regression coverage for conversation participant authorization, online STOMP delivery, offline Kafka fan-out, and offline notification event creation. Recorded notifications now publish through the `/user/queue/notifications` realtime gateway, and the repeatable `scripts/smoke-websocket.ps1` smoke covers authenticated STOMP connect, chat subscriptions, seller ack, buyer chat delivery, and buyer notification delivery. Full browser WebSocket smoke remains open.
...
- [x] Promotion usage persistence: `PromotionUsageRepositoryImpl` now writes durable rows to `promotion_usages` through a Flyway migration.
- [x] Seller coupon listing runtime error fixed by eagerly loading coupon applicable product ids.
- [x] Dev seed workflow: The `dev_seeds.sql` script seeds repeatable buyer, seller, admin, approved seller application, smoke products/variants, coupon, flash sale, default inventory rows.
...
- [~] Kafka, Redis, MongoDB, SMTP, OAuth, and VNPay have placeholder/default config in places; Docker Compose now provides local Kafka, Redis, MongoDB, Postgres, Mailpit, a gated dev OAuth callback, and a gated dev VNPay signed return path. Backend health smoke is `UP`; Elasticsearch adapter behavior now has regression coverage and configured username/password are wired into the REST client, but runtime Elasticsearch service search still needs validation. WebSocket delivery, real VNPay provider return, and full integration flows still need validation.
...
- `.\gradlew.bat :notification:notification-application:test --console=plain` passed after wiring recorded notifications to a realtime gateway, covering persistence followed by realtime delivery.
- `.\gradlew.bat :notification:notification-adapter:compileJava --console=plain` and `.\gradlew.bat :notification:notification-infrastructure:compileJava --console=plain` passed after the WebSocket controller implemented the realtime delivery port and the notification service bean accepted the new port.
- `scripts/smoke-websocket.ps1` now acts as a repeatable runtime smoke for seeded buyer/seller STOMP auth, `/user` queue subscriptions, seller chat ack, buyer chat delivery, and buyer notification delivery. It still requires a running local stack, so it is documented as a runtime smoke command rather than part of offline CI.
- `.\gradlew.bat :product:product-infrastructure:test --console=plain` passed after adding Elasticsearch adapter regression coverage for query body construction, hit/facet mapping, pagination metadata, and de-duplicated suggestions.
