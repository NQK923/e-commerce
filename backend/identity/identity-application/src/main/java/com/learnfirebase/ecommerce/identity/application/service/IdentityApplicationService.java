...
public class IdentityApplicationService implements RegisterUserUseCase, AuthenticateUserUseCase, OAuth2LoginUseCase,
        UserQueryUseCase, UpdateUserProfileUseCase, ListUsersUseCase, ManageUserAddressUseCase, RequestLoginOtpUseCase,
        com.learnfirebase.ecommerce.identity.application.port.in.RotateRefreshTokenUseCase, ResetPasswordUseCase {
    ...
    public com.learnfirebase.ecommerce.identity.application.dto.OtpChallengeDto request(String email) {
        return userRepository.findByEmail(email)
                .map(user -> otpService.issue(user.getEmail() != null ? user.getEmail().getValue() : email,
                        user.getId().getValue()))
    ...
    public void resetPassword(ResetPasswordCommand command) {
        ...
        User user = userRepository.findByEmail(command.getEmail())
                .orElseThrow(() -> new IdentityDomainException("User not found"));
        otpService.validate(command.getChallengeId(), command.getOtpCode(), user.getId().getValue(),
                command.getEmail());
    ...
    public UserDto execute(UpdateProfileCommand command) {
        ...
        User updated = User.builder()
                ...
                .shopDescription(
                        command.getShopDescription() != null ? command.getShopDescription() : user.getShopDescription())
                .shopBannerUrl(
                        command.getShopBannerUrl() != null ? command.getShopBannerUrl() : user.getShopBannerUrl())
                .createdAt(user.getCreatedAt())
                .updatedAt(Instant.now())
                .addresses(user.getAddresses())
                .build();
    ...
    private UserDto toDto(User user, String displayNameOverride) {
        ...
        return UserDto.builder()
                ...
                .createdAt(user.getCreatedAt())
                .addresses(user.getAddresses() != null ? user.getAddresses().stream().map(this::toAddressDto).toList()
